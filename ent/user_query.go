...
func (uq *UserQuery) QueryCar() *CarQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, true, user.CarTable, user.CarColumn),
		)
...
func (uq *UserQuery) loadCar(ctx context.Context, query *CarQuery, nodes []*User, init func(*User), assign func(*User, *Car)) error {
	...
	for _, n := range neighbors {
		fk := n.car_user
		if fk == nil {
			return fmt.Errorf(`foreign-key "car_user" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "car_user" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
